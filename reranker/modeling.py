...
class Reranker(nn.Module):
    def __init__(self, hf_model: PreTrainedModel, model_args: ModelArguments, data_args: DataArguments,
        ...
        self.hinge_loss_margin1 = torch.nn.MarginRankingLoss(margin=1.0, reduction='mean')
    ...
    def forward(self, batch, current_epoch=None):
        if self.training:
            if self.train_args.training_loss == 'LCE':
                ...
                logits = ranker_out.logits  # (batch_size, config.num_labels)
                ...
                loss = self.cross_entropy(scores, self.lce_target_label)
                ...
                return self.hf_model(**batch, return_dict=True)

            elif self.train_args.training_loss == 'hinge':
                ranker_out: SequenceClassifierOutput = self.hf_model(**batch, return_dict=True)
                logits = ranker_out.logits
                scores = logits.view(self.train_args.per_device_train_batch_size, 2)
                pos_scores = scores[:, 0]
                neg_scores = scores[:, 1]
                hinge_label = torch.ones_like(pos_scores)
                loss = self.hinge_loss_margin1(pos_scores, neg_scores, hinge_label)
                return SequenceClassifierOutput(
                    loss=loss,
                    **ranker_out,
                )

            elif self.train_args.training_loss == 'hinge_adaptive_score':
                ranker_out: SequenceClassifierOutput = self.hf_model(**batch, return_dict=True)
                logits = ranker_out.logits
                scores = logits.view(self.train_args.per_device_train_batch_size, 2)
                pos_scores = scores[:, 0]
                neg_scores = scores[:, 1]
                score_gap = 1. - torch.clamp(pos_scores.detach() - neg_scores.detach(), max=1.)
                loss = torch.mean(torch.clamp((neg_scores - pos_scores + score_gap), min=0), dim=0)
                return SequenceClassifierOutput(
                    loss=loss,
                    **ranker_out,
                )
                ...
                rank_gap = batch['rank_gap']
                ranker_out: SequenceClassifierOutput = self.hf_model(input_ids=batch['input_ids'],
                ...
                pos_scores = scores[:, 0]
                neg_scores = scores[:, 1]
                margin_gap = rank_gap.view(self.train_args.per_device_train_batch_size, 2)[:, 0]
                margin_gap1 = rank_gap.view(self.train_args.per_device_train_batch_size, 2)[:, 1]
                if not margin_gap.equal(margin_gap1):
                    raise ValueError

                # margin_gap = 1 + (margin_gap - 1) / 99
                margin_start = 0.2
                margin_rate = 0.1
                margin = margin_start + margin_rate * margin_gap
                loss = torch.mean(torch.clamp((neg_scores - pos_scores + margin), min=0), dim=0)
                return SequenceClassifierOutput(
                    loss=loss,
                    **ranker_out,
                )
