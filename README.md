...
We recommend you to create a new conda environment `conda create -n idem python=3.7`, activate it `conda activate idem`, 
...
the output ranking lists of victim NRM, which are used to train surrogate NRMs.
...
**a) Data Preparation:** 
Given a set of queries (e.g., 6,837 Eval queries in MS MARCO passage dataset) and their top-1K BM25 candidates by 
[Anserini](https://github.com/castorini/anserini) in the format of `query_id \t doc_id \t query_text \t doc_text \n`, 
we need to convert the raw text into the input features (refer to `topk_text_2_json.py`), and run re-ranking to obtain 
the outcomes of the victim NRM (refer to `bert_ranker_inference.sh`). After that, we can collect paired training samples 
from the top-k results (refer to `collect_surrogate_train_data_topk.py`), and convert each training sample into features 
(refer to `train_text_2_json.py`), in the format of the following example:
```
...
```
Note: To simulate the real situation, wherein the access to victim model is limited or unavailable, we choose to reduce 
the number of Eval queries (200 in our work) or use the Natural Question dataset (`biencoder-nq-train.json` released in 
[DPR](https://github.com/facebookresearch/DPR/blob/main/dpr/data/download_data.py#L45) for training instead 
(refer `collect_surrogate_train_data_nq.py`).

**b) Model Training:**
The surrogate NRMs are based on the pre-trained BERT-Base model, and we directly adopt the hinge loss with a margin of 1 
for ranking imitation (refer to `bert_ranker_train.sh`). After training, we can evaluate the performance of surrogate 
NRMs on the Dev set in MS MARCO passage dataset (refer to `bert_ranker_inference.sh`), and we can also compute the ranking 
similarities (e.g., the overlap of top-10 and the [Rank Biased Overlap](https://github.com/changyaochen/rbo)) to measure the 
...
We evaluate attack methods on randomly sampled 1000 Dev queries (in [Resources](https://github.com/cxa-unique/IDEM/#Resources)) 
...
models (like BART) in a blank-infilling pattern to generate connection sentences that include information about both 
...
`--surrogate_model` and `--surrogate_tag`. After running, there are two output files in the same folder for the `connect_sent_file`, 
...
3. Generated connection sentences and adversarial documents:
    ...
    - There are a total of 7 files in each compressed folder, one of them contains all connection sentences, other 2*3 files 
    contain the selected connection sentences and final adversarial documents under surrogate NRM S1 (`all-top29-ep2`), 
    S2 (`200-top29-ep2`) and S3 (`nq-p1n8-ep1`), respectively.
